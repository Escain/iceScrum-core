                ...
                hooks.each { hook ->
                    Hook.async.task {
                        ...
                        if (hook.secret) {
                            ...
                            requestHeaders.'x-icescrum-signature' = signature
                        }
                        if (log.debugEnabled) {
                            log.debug("hook (id:$hook.id) - request sent ${hook.url} for $eventToDisplay")
                        }
                        SimpleHttp.Response resp = SimpleHttp.post(hook.url, payload, requestHeaders,
                                grailsApplication.config.icescrum.hooks.httpTimeout as int,
